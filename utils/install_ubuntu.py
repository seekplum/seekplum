...
file name: install_ubuntu
author: hjd
create time:  2017-03-25 20:23
...
class Seekplum_Install(object):
    ...
    def set_service_autostart(self, service_name):
        """
        设置服务开机自启动
        :return:
        """
        ...
        # 设置开机启动(测试成功)
        # set_autostart = "update-rc.d supervisord defaults"
        # 删除开机启动
        # del_autostart "update - rc.d -f  supervisord remove"
        # 设置开机启动(测试成功)
        rc_local = "/etc/rc.local"
        with open(rc_local) as f:
            alias_content = f.read()
        start_cmd = "systemctl start %s" % service_name
        if start_cmd not in alias_content:
            # alias_bash = "sudo echo %s >> %s" % (start_cmd, rc_local)
            # 在指定行前面插入
            alias_bash = "sudo sed -i '/%s/i\%s' %s" % ("exit 0", start_cmd, rc_local)
            try:
                run(alias_bash)
    ...
    def check_os_version(self):
        ...
        recomm = "Recommended OS is Ubuntu 16.x"
        match = re.search(r"(\d+\.?\d*)", os_host)
        if match:
            version = match.group(1)
        else:
            version = ""
        if "ubuntu" in os_host.lower() and version.startswith("16."):
            print_ok(os_version)
            self.check_pass += 1
        else:
    ...
    def check_os_mem(self):
        ...
        recomm = "need no less 4G Memory ,Now Memory is %s G" % round(mem_size, 1)
        if mem_size >= 4:
            self.check_pass += 1
            print_ok(check_status=check_status)
        else:
    ...
    def cp_projects(self):
        if self.current_path != self.home:
            ...
            if is_exists:
                print_ok("copy seekplum-env success")
                sys.stdout.flush()
            else:
                print_error("copy seekplum-env failed")
                sys.stdout.flush()
        else:
            print_ok("seekplum-env no need to copy")
            sys.stdout.flush()
        for file in self.file_list:
    ...
    def set_mysql_autostart(self):
        ...
        rm_on = "sudo rm -f %s" % mysql_init
        start_list.append(rm_on)

        # 2.重新复制mysql.server
        cp_on = "sudo cp %s %s" % (self.mysql_server, mysql_init)
        start_list.append(cp_on)
        # 设置执行权限
        chmod_mysql = "sudo chmod 755 %s" % mysql_init
        start_list.append(chmod_mysql)
        # 设置执行权限
        chmod_mysql = "sudo chown %s:%s %s" % (self.username, self.username, mysql_init)
        start_list.append(chmod_mysql)
        ...
        cmd = "sudo echo %s > /etc/ld.so.conf.d/mysql-seekplum.conf" % (os.path.join(self.mysql_home, "lib"))
        ...
        for cmd in start_list:
            try:
                ...
                is_ok = False
        self.set_service_autostart("mysql")
        if is_ok:
    ...
    def start_mysql(self):
        """启动mysql"""
        # 启动前，先杀死老的mysql进程
        cmd = "sudo pkill -9 mysql"
        run(cmd)
        # 消除警告
        cmd_canel = "systemctl daemon-reload"
        run(cmd_canel)
        # 启动
        cmd_start = "systemctl restart mysql > /dev/null"
        os.system(cmd_start)
        logger.info("cmd: %s" % cmd_start)
        cmd_status = "systemctl status mysql"
        try:
            output = run(cmd_status)
        except Exception as e:
            print_warn(e.message)
        T = 60
        while "(dead)" in output and T >= 0:
    ...
    def set_hostname(self, check_again=False):
        ...
        if not match:
            if check_again:
                ...
                run("sudo echo 127.0.0.1 `hostname` >> /etc/hosts")
    ...
    def set_alias(self):
        ...
        try:
            now_home = run("sudo echo $HOME").strip()
            no_such_home = False
        except Exception as e:
            print_error(str(e))
            no_such_home = True
        ...
        if not no_such_home:
            ...
            for alias in alias_list:
                ...
                if res[0] not in alias_content:
                    alias_bash = "sudo echo %s >> %s" % (alias, bashrc)
                    try:
                        run(alias_bash)
                    except Exception as e:
                        print_error(str(e))
    ...
    def start_supervisor(self):
        """ 启动superviord """
        # 清除supervisor.sock文件
        cmd = "sudo pkill -9 supervisord"
        try:
            run(cmd)
        except:
            pass
        try:
            sock = run("sudo find /tmp -name *supervisor.sock")
            ret = sock.split('\n')
            unlink_ok = True
            for res in ret:
                if res:
...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/home/hjd/seekplum-env/bin
CONF=/home/hjd/seekplum-env/packages/conf/supervisor/supervisord.conf
DAEMON=/home/hjd/seekplum-env/bin/supervisord
NAME=supervisord
DESC=supervisor
...
LOGDIR=/home/hjd/logs/supervisor
...
DAEMON_OPTS="-c $CONF $DAEMON_OPTS"
...
exit 0
"""
        supervisord_path = '/etc/init.d/supervisord'
        ...
        self.set_service_autostart("supervisord")
