...
Example:
    ...
    colored_name = "\n".join([
        get_color_string(bcolors.RED, "Mr"),
        get_color_string(bcolors.GREEN, "Baptiste"),
        get_color_string(bcolors.BLUE, "Clement")])
...
class Texttable:
    ...
    def _splitit(self, line, isheader):
        ...
        for cell, width in zip(line, self._width):
            ...
            for attr in bcolors_public_props():
                cell = cell.replace(getattr(bcolors, attr), '').replace(bcolors.ENDC,'')
            old_cell_list = original_cell.splitlines()

            lost_color = bcolors.WHITE

            find_end = False
            for i,c in enumerate(cell.splitlines()):
                old_c = old_cell_list[i]
                for attr in bcolors_public_props():
                    if old_c != c and old_c.replace(getattr(bcolors, attr), '').replace(bcolors.ENDC,'') == c and attr != "ENDC":
                        if getattr(bcolors, attr) in old_c:
                            if attr == "WHITE":
                                if find_end:
                                    lost_color = attr
                            else:
                                lost_color = attr
                if getattr(bcolors, "ENDC") in old_c:
                    find_end = True
...
if __name__ == '__main__':
    ...
    table.add_rows([ [get_color_string(bcolors.GREEN, "Name Of Person"), "Age", "Nickname"],
                     [get_color_string(bcolors.BLUE,"Mr")+get_color_string(bcolors.RED,"\nClement"), 1, get_color_string(bcolors.RED,"Baby")] ])
    import pdb
    pdb.set_trace()
    print(table.draw() + "\n")
